/**
 * Quick sort test functions using WASM.
 */
import { readFile } from "node:fs/promises";
import path from 'path';
import { fileURLToPath } from 'url';

export default class QuickSortTest  {
    /**
     * Fetch and load the WASM file.
     * @return {Promise} A promise.
     */
    async load() {
        // Get current folder
        const __filename = fileURLToPath(import.meta.url);
        const __dirname = path.dirname(__filename);
        
        // Read WASM data
        const wasmBuffer = await readFile(__dirname + '/quick-sort.wasm');

        // Create WASM data from buffer
        const wasmData = new Uint8Array(wasmBuffer);

        // Create memory
        this.memory = new WebAssembly.Memory({ initial: 1, maximum: 2 });

        // Set options
        const options = {
            import: {
                memory: this.memory
            }
        }

        // Instantiate the WASM data
        const promise = WebAssembly.instantiate(wasmData, options)
        .then((resultObject) => {
            // Set the module object
            this._module = resultObject.module;

            // Set the instance object
            this._instance = resultObject.instance;
        });

        // Return the promise
        return promise;
    }

    /**
     * Run the quick sort function.
     * @param {Number} dataOffset Where in the memory does the list of bytes exist.
     * @param {Number} dataItemCount The number of byte items in the list.
     */
    runQuickSort(dataOffset, dataItemCount, swapOffset) {
        // Call the WASM runQuickSort function
        this._instance.exports.runQuickSort(dataOffset, dataItemCount, swapOffset);
    }
}
